...
describe("query reads", () => {
  ...
  test("session summaries do not use generated context when no human prompt exists", () => {
    const db = freshDb();
    db.exec(`
      INSERT INTO session(id, tool, source_session_id, title, started_at, is_subagent)
      VALUES
    ...
    expect(listSessions(db, { limit: 10 }).map((session) => session.title)).toEqual([null, null]);
    db.close();
  });
