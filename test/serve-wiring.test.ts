...
async function withServe<T>(c: ServeCase, run: (port: number) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const match = buffer.match(/serving http:\/\/[^:]+:(\d+)/);
      if (match) {
        port = Number(match[1]);
        break;
      }
...
describe.skipIf(lanIP == null)("serve trusted-peer wiring (real CLI process)", () => {
  ...
  test("env stays honored when flags are also passed (union, the original 403 bug)", async () => {
    await withServe(
  ...
  test("flags stay honored when the env var is also set (union, other direction)", async () => {
    await withServe(
