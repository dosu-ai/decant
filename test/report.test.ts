...
describe("static report rendering", () => {
  ...
  test("CTA respects the Dosu suggestions setting without hiding attribution", () => {
    ...
    expect(html).toContain('aria-label="Optimized with Dosu"');
    expect(html).not.toContain("Dosu keeps agent context fresh automatically");
    expect(html).not.toContain("utm_content=report_cta");
